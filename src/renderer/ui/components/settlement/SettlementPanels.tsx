...
import type { SettlementResult, DiceCheckState } from '../../../core/types';
...
export function SettlementSummaryView({ results }: { results: SettlementResult[] }) {
  const setScreen = useUIStore(s => s.setScreen);

  return (
    <div className="h-full p-6 overflow-auto">
      <h2 className="text-xl font-bold text-gold mb-6 text-glow-gold">结算总览</h2>
      {results.length === 0 && <div className="text-gold-dim text-center py-12">无结算结果。</div>}
        ...
        {results.map((result, idx) => (
          <Panel key={idx} variant="dark" title={result.scene_id}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs px-2 py-1 bg-ink-light rounded text-gold-dim">{result.settlement_type}</span>
            </div>
            <p className="text-sm text-parchment/70 mb-4 italic">{result.narrative}</p>
            {result.dice_check_state && (
              <div className="mb-4">
                <DiceResult dice={result.dice_check_state.dice} explodedStartIndex={3} successThreshold={7} />
...
export function SettlementDiceRerollModal({
  diceState,
  rerollAvailable,
  onAccept,
  onReroll,
}: {
  diceState: DiceCheckState;
      ...
      <Panel variant="dark" title="收骰再断" className="w-full max-w-2xl rounded-[24px] border-[#8a6d2b]/35 bg-[linear-gradient(180deg,rgba(20,12,8,0.94),rgba(12,8,6,0.98))]">
        <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-4">
            <div className="text-sm text-gold-dim font-(family-name:--font-body)">可用重投次数：{rerollAvailable}</div>
            <div className="rounded-[16px] border border-gold-300/20 bg-ink-light/30 p-4 text-center">
              <DiceResult
                dice={diceState.dice}
                rerolledIndices={diceState.rerolled_indices ?? []}
                explodedStartIndex={3}
                successThreshold={7}
              />
              <div className="mt-3 text-sm text-parchment-100 font-(family-name:--font-body)">
                {diceState.dice.join(' + ')} {diceState.modifier >= 0 ? '+' : '-'} {Math.abs(diceState.modifier)} = {diceState.total} / DC {diceState.dc_with_offset}
              </div>
            </div>
          </div>
          <div className="rounded-[18px] border border-gold-300/14 bg-[linear-gradient(180deg,rgba(201,168,76,0.07),rgba(26,15,10,0.24))] p-5">
            ...
            <div className="mt-5 flex justify-end gap-3">
              ...
              <Button variant="primary" size="sm" glow onClick={onReroll}>
                全部重投（剩余{rerollAvailable}次）
              </Button>
            </div>
          </div>
        </div>
