...
if not os.environ.get("OPENAI_API_KEY", "").strip():
    print("ERROR: OPENAI_API_KEY is not set. Check .env file.", file=sys.stderr)
    sys.exit(1)
...
RARITY_VISUAL_ENHANCEMENTS: Dict[str, Dict[str, str]] = {
    "common": {
        "rarity_palette": (
            "Color palette: muted taupe, weathered earth, soft chalk white. "
            "Simple construction, plain materials, no decorative elements, no glow effects. "
        ),
    },
    "rare": {
        "rarity_palette": (
            "Color palette: cool cyan-blue, misted ash gray, cold jade tint. "
            "Refined craftsmanship with subtle metallic sheen, light engravings, faint cool-toned aura. "
        ),
    },
    "epic": {
        "rarity_palette": (
            "Color palette: deep violet, ember red, dusky aureate glow. "
            "Elaborate ornamental details, inlaid gemstones, visible radiant halo, intricate carvings and filigree. "
        ),
    },
    "legendary": {
        "rarity_palette": (
            "Color palette: luminous pearl white, flowing iridescence, celestial rosy haze. "
            "Supremely ornate, luminous materials with inner glow, ethereal energy streams, supernatural visual impact, "
            "every surface richly detailed with mythical motifs. "
        ),
    },
}
...
你是熟读武侠小说《雪中悍刀行》的物品设计专家。

## 输入
- 物品名、类型(weapon/armor/accessory/mount)
- 稀有度：Common(平凡) / Rare(稀有) / Epic(精英) / Legendary(传奇) 4个等级之一
- 可选补充描述(bio)

## 要求
为该物品生成 4 种不同的物理设计方案。
⚠️ 重要：物品名只是一个名称代号（如"木马牛"只是一把剑的名字），不代表物品的实际外形。
物品的实际形态由类型（weapon/armor/accessory/mount）和补充描述(bio)决定。
绝对不要从物品名的字面含义推测物品外形。
...
""",
    ...
    "rarity_palettes": {
        rarity: values["rarity_palette"]
        for rarity, values in RARITY_VISUAL_ENHANCEMENTS.items()
    },
}
...
def _load_item_prompt_config() -> Dict[str, Any]:
    ...
    if ITEM_PROMPT_CONFIG_PATH.exists():
        try:
            with open(ITEM_PROMPT_CONFIG_PATH, "r", encoding="utf-8") as f:
            ...
            persisted_palettes = persisted.get("rarity_palettes", {})
            if isinstance(persisted_palettes, dict):
                for rarity in config["rarity_palettes"]:
                    if isinstance(persisted_palettes.get(rarity), str):
                        config["rarity_palettes"][rarity] = persisted_palettes[rarity]
        except (json.JSONDecodeError, OSError):
            pass
    return config
...
def _save_item_prompt_config(config: Dict[str, Any]) -> Dict[str, Any]:
    normalized = {
        "variant_system_prompt": config.get(
            "variant_system_prompt",
            DEFAULT_ITEM_PROMPT_CONFIG["variant_system_prompt"],
        ),
        "style_template": config.get(
            "style_template",
            DEFAULT_ITEM_PROMPT_CONFIG["style_template"],
        ),
        "rarity_palettes": {
            rarity: config.get("rarity_palettes", {}).get(
                rarity,
                DEFAULT_ITEM_PROMPT_CONFIG["rarity_palettes"][rarity],
            )
            for rarity in DEFAULT_ITEM_PROMPT_CONFIG["rarity_palettes"]
        },
    }
    with open(ITEM_PROMPT_CONFIG_PATH, "w", encoding="utf-8") as f:
...
def _build_item_prompt(description: str, rarity: str = "rare") -> str:
    ...
    no_text = templates.get("no_text_constraint", ga.NO_TEXT_CONSTRAINT)
    rarity_palette = (
        prompt_config.get("rarity_palettes", {}).get(rarity)
        or RARITY_VISUAL_ENHANCEMENTS.get(rarity, RARITY_VISUAL_ENHANCEMENTS["rare"])["rarity_palette"]
    )
    template = (
        prompt_config.get("style_template")
        or templates.get("item_template")
        or ITEM_STYLE_B_TEMPLATE
    )
    return template.format(
        description=description,
...
def _generate_item_descriptions_blocking(
    client, name: str, equipment_type: str, bio: str, rarity: str = "common"
) -> List[str]:
    ...
    user_msg = (
        f"类型：{eq_type_cn}\n"
        f"稀有度：{rarity_cn}\n"
        f"物品名（仅代号，不代表外形）：{name}\n"
        f"{bio_section}"
        f"请基于《雪中悍刀行》世界观（如物品在原著中存在），"
        f"为该物品生成 4 条不同物理设计方案的 description，以 JSON 数组格式输出。"
    )
    prompt_config = _load_item_prompt_config()
    response = client.responses.create(
        model=DESCRIPTION_MODEL,
        instructions=prompt_config.get("variant_system_prompt", _ITEM_VARIANT_SYSTEM_PROMPT),
        input=user_msg,
        temperature=0.9,
        max_output_tokens=800,
    )
...
async def create_item(body: CreateItemRequest) -> Dict[str, Any]:
    ...
    try:
        descriptions = await loop.run_in_executor(
            None, _generate_item_descriptions_blocking, client, body.name, body.equipment_type, body.bio, body.rarity
        )
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"AI generation failed: {exc}")
...
async def regenerate_item_variants(item_name: str, body: RegenerateItemVariantsRequest) -> Dict[str, Any]:
    ...
    try:
        descriptions = await loop.run_in_executor(
            None, _generate_item_descriptions_blocking, client, item_name, equipment_type, body.bio, rarity
        )
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"AI generation failed: {exc}")
...
async def generate_item_images(body: GenerateRequest):
    ...
    async def event_stream():
        ...
        # Look up item rarity from profile (default to "rare" if not found)
        profiles = _read_item_profiles()
        item_rarity = profiles.get(body.name, {}).get("rarity", "rare")
